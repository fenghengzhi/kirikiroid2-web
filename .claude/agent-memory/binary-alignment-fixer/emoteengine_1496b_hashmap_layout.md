...
**Why:** local EmoteEngine.h once mis-modeled bytes 824..991 / 1272..1439 as `uint8_t[]` placeholder blocks (_inlineVectorBlocks_*, _scalarField_824..864, _residual_*). They are actually 7 libstdc++ inline unordered containers + 4 `std::vector<ttstr>`. The local `_bindListHead@1456` pseudo-field physically IS HM#7's `_M_before_begin._M_nxt`.
...
- 800..823  : std::vector<ttstr> mirror match patterns (begin/end/cap). dtor releases each string element.
...
- 992..1063 : 3x std::vector<ttstr> @992/1016/1040 (timeline normal/diff/active labels). dtor releases each string element.
...
- 1160      : byte selectorEnabled = 1（ctor 的 32-bit store 同时覆盖邻接 flag；独立 getter/setter 只按 byte 访问）。
...
- 1328..1383: HM#5 unordered_map<ttstr,EmoteVariableRange>; its 64B node owns key/value ttstr plus four doubles. clear helper sub_68577C releases both strings before deleting each node. The former "1288/1272 vector pair" note was a stale offset interpretation and is superseded.
...
2026-07-18 correction: HM#1/HM#2 are the positive/negative mirror caches; HM#3 is
`unordered_map<ttstr,EmoteHM3Value>` with owning raw timeline element,
`EmoteTimelineData80B*`, 56B track deque/24B frame vectors and runtime animator fields;
HM#4 is the instant-variable ttstr set; HM#5 maps labels to `EmoteVariableRange`;
HM#6 maps labels to
`EmoteVarRef{type,index}`; HM#7 maps labels to double. Exact semantic names and current
typed declarations live in `EmoteEngine.h`; the old blanket "all six value types OPEN"
