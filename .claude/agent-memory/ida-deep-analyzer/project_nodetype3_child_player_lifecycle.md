...
description: nodeType=3 child Player 完整生命周期(创建/释放/dtor级联)+ child+8 非拥有反向链 + re-init 同 this 不是 new
metadata:
  type: project
---

nodeType=3 sub-player 在二进制中**全程 TJS-refcount + C++ dtor 级联**释放,无泄漏、无环。
...
- 0x6B43DC `STP X8,X20,[X21]`: child+0=parent vtable word, **child+8=parent this 裸指针(无 AddRef)**。
- 创建 child 的 NCB dispatch via `sub_6F1794`(CreateAdaptor→QueryInterface dword_1AB8848→native instance,设 instance+8=child),AddRef,存入 **node+1912** tTJSVariant(0x6B46E0 sub_A0FB64 CopyRef,覆盖旧值)。
...
**child+8 不成环**: 裸存,Player_dtor 从不 Release this+8。子树唯一拥有边=parent node+1912→child dispatch(单向)。
