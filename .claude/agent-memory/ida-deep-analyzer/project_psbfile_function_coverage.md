...
description: PSBFile.dll corrected 108-function boundary and exhaustive grouped manifest through the typed NCB tail
metadata:
  type: project
---
...
range through `0x59B708`, for 108 functions total. The next function at
`0x59B7E8` is a generic `std::vector<std::string>` instantiation with only
PLT/data xrefs and no call from the PSBFile tail.

The extra 19 functions are native-holder construction/three destructors,
