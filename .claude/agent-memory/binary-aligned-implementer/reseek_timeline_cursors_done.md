...
**How to apply:** layer coarse scan ≠ seekLayerEventStreamLike_0x6B6ADC (that's the incremental advance/rewind form). Modelled on B's PlayerFrameStepping.cpp:482-600 reference, ported to real Player fields (_layerFrameCursor/_layerCurTime/_layerNextTime/_rootFrameCursor/_rootCurTime/_rootNextTime/_rootContent). Gate reuses live `_syncActive`/_motionCompleted/_clampedEvalTime/_frameTickCount/_syncWaiting/_pendingEvents. int-truncation of layer curTime/nextTime is LOAD-BEARING (binary uses propGetInt, root uses propGetDouble).
