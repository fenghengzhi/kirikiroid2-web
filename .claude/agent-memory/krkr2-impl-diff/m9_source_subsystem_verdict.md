...
- **loadSource 0x6A7BA8 std::list (+72/+80 sentinel links)**: match loop uses
  strict full Variant key, src ttstr and blendMode; color is compared separately.
  On mismatch it rebakes the same Layer and performs node copy-to-front plus old
  node erase. The inherited local NCB method now has the exact
  `(iTJSDispatch2 *source,iTJSDispatch2 *descriptor)->Layer` signature. The
  separate Web-only `Player.loadSource(name)` helper is extra compatibility code,
  not the NCB implementation and does not populate this list.
...
- **clearCache 0x6A8438**: dispatch-invalidates each Layer, clears only the +72
  list and zeros current bytes; it does not touch hashmaps.
...
**RM ctor sub_6A88CC FIRST instruction (0x6a88f8) calls SourceCache ctor
sub_6A78F4() on the SAME a1 object**, initializing primary/buffer Layers, current
bytes, byte limit, and the libstdc++ `std::list` sentinel links at +72/+80. The
older `+64 layerType` and custom-intrusive-list labels were disproved. This is
`class ResourceManager : public SourceCache`; the three shared NCB members are
base members re-listed on the derived table.
