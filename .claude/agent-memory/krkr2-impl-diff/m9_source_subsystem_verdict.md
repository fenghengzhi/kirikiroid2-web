...
description: M9 source subsystem verdict, corrected again 2026-07-23. Raw mapped-record/ObjSource work remains valid; the former full source-path closure missed 0x695DE8's render-time caller and the direct SourceState alias.
metadata:
  type: project
---
...
- **loadSource 0x6A7BA8 list (+72/+80)**: match loop 0x6a8004-0x6a8074 keys by (key sub_A10428 @v27+2, src ttstr @v27[7], blendMode @*(v27+16)) — COLOR NOT in match key. Color (v27+17..+20) is compared separately @0x6a80d4, updated on mismatch, then re-baked through sub_6A6BE0 and moved to the front. 2026-07-23 correction: the production prepared-item route now uses exact `(key,src,blendMode)` identity, passes the incoming source object only to the miss/color-change bake, and forbids storage-path fallback. The older generic/by-name facade still accepts `key || resolved storage key`; it is explicitly legacy and must not be cited as a faithful implementation of the NCB `(source,descriptor)` method.
...
**RM ctor sub_6A88CC FIRST instruction (0x6a88f8) calls SourceCache ctor sub_6A78F4() on the SAME a1 object**, initializing SourceCache fields +20 (primaryLayer)/+36/+40 (bufLayer Layer)/+60 (current cache bytes)/+64 (cache byte limit)/+72/+80 (intrusive list head). RM ctor then initializes its OWN fields from +88 (HashMap A) up. The older `+64 layerType` label was disproved by trim@0x6A6B08 and corrected 2026-07-23. Signature of **RM being a DERIVED class of SourceCache (or embedding it as the [0,88) prefix subobject)** — binary `class ResourceManager : public SourceCache`. The 3 shared loadSource/clearCache/bufLayer members are BASE-class members re-listed on derived RM's NCB table (normal C++ NCB inheritance), NOT mere address-sharing between unrelated classes.
...
### 2026-07-23 correction to the closure claim
The raw owner/map migration above remains valid, but it was not a complete call-chain
closure. Fresh xrefs found `sub_6F1060@0x6F1060` as a second caller of
`sub_695DE8`; `sub_6C2334@0x6C360C` stores a persistent `SourceState*` in each
prepared item, and `sub_6ADFBC@0x6AE154..0x6AE188` rereads its rect after the
texture getter. The local snapshot-only path missed that alias/data flow. The shared
helper, alias and post-getter read are now restored, along with the branch-local
resource calls and atlas geometry. `0x6D5C68` keeps its direct `sub_6F67CC`
texture getter, while only `0x6ADE24` uses `sub_6F1060`; both `0x6F1060` and
PrivateMotionGLL `0x6DE738` pass `SourceState.object` to `0x6C1B70` without
consulting `source.path`. Do not treat this file as a global 100% proof beyond
the named audited functions.
