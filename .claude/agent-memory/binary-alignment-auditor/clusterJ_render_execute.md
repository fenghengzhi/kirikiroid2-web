...
**2026-07-23 状态修订：** 下列清单记录的是 2026-06-07 当时快照；J1/J4/J6/J9 已由后续渲染管线重构关闭，J5 也在本轮把生产字段与 harness 一并改成 `std::array<float,4>`。不得再把这些历史项当作当前缺口。

**本地偏差(PlayerRenderExecute.cpp，历史快照)**:
...
- ✅ J5（2026-07-23）: 本地 `clipRect` 与 harness 均为 `std::array<float,4>`，对齐二进制 float[4]。
