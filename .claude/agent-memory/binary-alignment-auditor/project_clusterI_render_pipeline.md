...
**skipFlag1 VERDICT = ALIGNED**: binary sub_6C2334@0x6c33c0 item+18 = `(a6&1)?1:(node+48!=0)` = inheritedFlag18||(node+48!=0). node+48 PROVEN=priorDraw: sub_6BC4F0@0x6bc6c4 `node+48=sub_6636D4(emoteEdit,"priorDraw")&1` only if node+1996(forceVisible) else 0@0x6bc67c (byte 0/1). Consumed @0x6c7630 renderToCanvas preview-gated(a1+1096) skip. Local PlayerRenderItems.cpp:477 `skipFlag1=!(inheritedFlag18||(node.priorDraw!=0))` EXACT (negated sense). a6 prop line259 `_renderItemInheritedFlag18||nodePriorDraw` matches. Residual m2logo items[1] frame12+ build_flow mismatch is NOT a formula bug — likely phase placement (I1) or per-frame priorDraw/forceVisible timing.
...
**Item layout**: binary item = operator new(0x1B0)=432B raw block, alloc @0x6c2754. Key offsets: +16 rawFlag16=node+201, +17 skipFlag0=((preview?1097:1089)&(1<<nodeType))==0, +18 skipFlag1, +19 drawFlag=node+1960?1:node+1961, +20 layerId-latch, +21 drawable-clip, +136..164 corners=node+1856..1884, +184..196 paintBox=node+1888..1900, +200..212 viewport=node+1936 chain, +216..228 clipRect (FLOAT in binary; local uses int[4] — type mismatch), +232 opacity=node+1576, +244 stencilComposite=node stencilType, +248 context=player+1012, +280 meshType=node+2000, +304 leafLayer, +324 composedLayer, +424 layerId. Local splits into STL NativeRenderItemFields+PreparedRenderItem (vector/variant/string) = standing container divergence (always ⚠ per CLAUDE.md).
