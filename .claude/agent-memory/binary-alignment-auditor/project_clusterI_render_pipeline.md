...
**KEY re-mapping（2026-07-23 corrected）**: sub_6C4E28 (renamed Player_emitRenderItem_requireLayer) is the leaf/group pre-walk called from Player_renderToCanvas@0x6c756c under `!priorDraw` and from sub_6C9CA8@0x6c9e74. It performs clip + requireLayerId (item+424/item+20) + leaf TJS copy, then group compose. Local `buildRenderCommands` is now called at that exact renderToCanvas pre-walk point and owns those two loops；its historical name does not place the work in item-construction function 0x6C2334. The former “build→execute phase divergence” verdict is false and superseded.
...
**rawFlag20 VERDICT = aligned in the 0x6C4E28 pre-walk**: binary and current local both latch item+20=1 only at LABEL_28 semantics (`drawFlag19 && drawable && item+20==0`) during the renderToCanvas pre-walk.
