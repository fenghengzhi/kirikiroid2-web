...
对于 PSB/MTN 资源处理：
- `PSBRawNode_GetResource_guess@0x5996E4` = raw-node 资源 helper（返回借用指针 + 设置大小）；二进制没有保留其精确源码名，禁止写成确定的 `PSB_getResourceData`
...
**核心原则：命名权威是二进制自身保留的名字证据。** NCB 注册字符串、
RTTI/typeinfo、导出符号和字符串常量优先于本地代码；本地代码只能作交叉参照，
不能反向证明二进制里的源码名。二进制和本地都没有精确名字证据时必须加
`_guess` 后缀，禁止仅从行为猜名字。
...
| `0x5996E4` | `PSBRawNode_GetResource_guess` | raw-node 资源借用指针 + 大小；精确源码名未知 |
