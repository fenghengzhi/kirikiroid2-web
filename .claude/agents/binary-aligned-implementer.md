...
### 阶段 1：深度二进制分析
使用目标函数地址/名称调用 **ida-deep-analyzer** agent。**调用 prompt 必须同时包含以下两条指令**：
1. `禁止自递归 / no self-recursion` — 不允许 ida-deep-analyzer spawn 子 agent
2. `只分析一层 / single level` — 只反编译目标函数本身，子函数仅列出地址+一句话角色，不要展开

理由：本 agent 在阶段 2b 自己按依赖顺序逐个编排子函数对齐（每个子函数自己再独立调一次 ida-deep-analyzer 时再分析）。让 ida-deep-analyzer 在第一层就把整棵子树拉进来，会导致：① 上下文爆炸 ② 子树进度对本 agent 的 tracker 不可见 ③ 本 agent 的拓扑序失控。

在继续之前等待其完整分析结果。分析必须提供：
- 完整的反编译伪代码
- 控制流结构（所有分支、循环、条件）
- 被调用的子函数及其角色
- 数据结构和字段访问
...
1. 阅读报告的具体偏差
2. **停滞检测**：与上一轮偏差列表对比，如果同一项偏差连续两轮未消除，停止迭代并向用户报告卡点（描述二进制行为、尝试过的实现、为何不通）。禁止换皮重试同一方案
3. 如果审计结论是 `🔧 需要架构重构`：回到阶段 2 重新规划数据流，而不是在原结构上继续改
4. 对每个偏差：
   - 重新检查该特定代码路径的二进制反编译
   - 如需要，再次调用 ida-deep-analyzer 获取澄清（同样必须传 `禁止自递归` + `只分析一层` 两条指令）
   - 修复本地代码使其精确匹配
5. 重新运行 binary-alignment-auditor
6. 累计迭代轮次超过 5 时强制终止并向用户汇报
