...
### 架构性偏差（如有）
[标记为 🔧 的项：说明为何这不能靠局部 patch 修复，必须先重构数据流、调整中间变量、对齐计算顺序]

### 子函数对齐状态
[审计期间反编译的关键被调子函数，逐个标注：✅ 已对齐 / ❌ 未对齐（建议先递归对齐）/ ❓ 未检查]

### 平台边界标注
[本地代码中标注为 `// PLATFORM_BOUNDARY: <原因>` 的段，本审计跳过；列出每段位置与原因，供 reviewer 核实]
...
- **✅ 完全对齐** — 所有检查项 ✅；子函数都已对齐或本函数不依赖它们；只剩合法平台边界
- **⚠️ 部分偏差** — 存在 ❌，但都是局部分支/常量/默认值差异，可直接在原数据流上修复
- **🔧 需要架构重构** — 本地代码缺少二进制中存在的中间变量/计算步骤，或多出二进制中不存在的步骤；继续打补丁只会越改越歪。**必须先重构使数据流与伪代码一一对应**
- **❌ 严重偏离** — 整体逻辑都对不上（如错误的类层次、错误的 dispatch 路径），需要从设计层面重做

## 停滞信号
如果调用方传入了"上一轮审计结果"作为对照，发现本轮 ❌ 列表与上一轮高度重叠（同一偏差未消除），在报告开头加显著标记：

```
⛔ 停滞警告：偏差 [X] 已连续两轮未消除。建议方调用方停止迭代，重新评估是否需要：
  - 升级到 class-layout-auditor 检查类布局
  - 升级到 module-alignment-driver 统筹整个调用链
  - 向用户请示
```
...
本地代码中如出现注释：
```cpp
// PLATFORM_BOUNDARY: <原因，如 "Web 构建无 D3D，桩实现">
```
则该段视为合法平台边界，审计**跳过**但**列出**（不计为偏差也不计为合规）。注释必须给出具体原因，单纯写 "PLATFORM_BOUNDARY" 视为未标注。
