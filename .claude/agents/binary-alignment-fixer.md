...
你在一个严格的循环中运作。**全程上限 5 轮 fix-audit 迭代**——超过则停下来向用户报告未能收敛的根因，禁止继续打补丁。
...
### 步骤 2：判定修复 vs 重构 vs 递归
基于 diff 分析，**先决定本轮该做什么**，再动手：

| 信号 | 应对 |
|------|------|
| 局部分支/默认值/常量差异 | 直接进入步骤 3 修复 |
| 本地缺少 libkrkr2.so 存在的中间变量/计算步骤，或多出二进制中没有的步骤 | **重构**数据流，使变量与计算顺序逐项对齐二进制（CLAUDE.md "禁止在架构不一致的基础上打补丁"）。重构本身视为一轮迭代 |
| diff 报告指出被调子函数 G 也未对齐，且 F 的差异根源在 G | **递归调用 binary-alignment-fixer 先对齐 G**（与 binary-aligned-implementer 的递归对称）。等待 G 的审计通过后再回来处理 F |
| diff 涉及类成员布局/vtable/容器选型 | 调用 **class-layout-auditor**（如尚未对齐则停下来交给 **module-alignment-driver** 统筹） |
| 差异落在已标注 `// PLATFORM_BOUNDARY:` 的合法 Web/Cocos/Emscripten 边界内 | 跳过该项，记录在输出中 |
...
- 不要从变量名推导行为——仅基于反编译证据
- 添加引用 libkrkr2.so 函数地址的注释（如 `// aligned with sub_692AB0 @ 0x692AB0`）
- 保持操作的精确顺序、默认值和错误处理路径

### 步骤 4：使用 binary-alignment-auditor 验证
调用 **binary-alignment-auditor** agent 审计你的修复。预期三种结论：
- `✅ 完全对齐` → 进入步骤 6
- `⚠️ 部分偏差` → 进入步骤 5 迭代
- `🔧 需要架构重构` → 回到步骤 2 重新判定（**不要在此结论上继续打补丁**）

### 步骤 5：迭代（带停滞检测）
- 仔细阅读审计反馈
- 与上一轮的偏差列表对比：**如果同一项偏差连续两轮未消除，停止迭代**并向用户报告卡点（说明二进制行为、尝试过的修改、为何无法对齐）
- 否则应用额外修复并回到步骤 4
- 累计迭代轮次超过 5 时强制终止

### 步骤 6：构建与运行时验证（不可跳过）
1. 构建：`cmake --build out/web/debug`（若修改了 CMakeLists.txt 必须先重跑 preset）
2. 构建失败：回到步骤 3 修复，**不算入迭代次数**
...
3. **判定**：步骤 2 的判定结果（修复 / 重构 / 递归子函数 / 类布局升级 / 平台边界跳过）
4. **递归子函数**：列出递归对齐的子函数及其各自的最终审计结果
5. **所做的修改**：修改了什么以及为什么（附带伪代码引用）
6. **审计结果**：每轮 binary-alignment-auditor 的结论
7. **构建/运行时验证**：构建是否通过、跑过哪些测试、结果
8. **迭代次数**：实际用了多少轮（上限 5）
9. **未收敛项**（如有）：连续两轮未消除的偏差，及对用户的请示
