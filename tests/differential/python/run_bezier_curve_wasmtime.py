#!/usr/bin/env python3
"""Architecture-neutral Bezier-curve Wasmtime verifier."""
...
def drive_cases(wasm_path: Path, spec_dir: Path) -> dict:
    ...
    x_ptr = exports["get_curve_x_ptr"](store)
    y_ptr = exports["get_curve_y_ptr"](store)
    ...
    for spec in load_specs(spec_dir):
        ...
        write_doubles(base, x_ptr, xs)
        write_doubles(base, y_ptr, ys)
        run_fn(store, len(xs), spec["t"])
        results.append({
            "case_id": spec["id"],
    ...
    return {
        "ok": True,
        "runner": "bezier-curve-wasmtime-direct-export",
