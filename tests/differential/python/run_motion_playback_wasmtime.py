...
def _build_state_trace_xp3(specs: list[dict], work_dir: Path) -> Path:
    from oracle_runner.adapters import motion_playback as mpb

    repo_root = REPO_ROOT
    xp3pack = repo_root / "tools" / "bin" / "mac" / "rel" / "xp3pack"
    if not xp3pack.exists():
        raise FileNotFoundError(
            f"xp3pack not found at {xp3pack}. Build the native xp3pack tool "
            "first."
        )

    specs_by_id = {str(spec["id"]): spec for spec in specs}
    lines = [
        "// Generated by run_motion_playback_wasmtime.py for state tracing.",
        "// draw(base) is intentionally omitted: Wasmtime headless draw stops",
        "// this deterministic startup after the first progress() iteration,",
        "// while the LLDB state sample is taken immediately after progress().",
        "Plugins.link(\"motionplayer.dll\");",
        "Plugins.link(\"emoteplayer.dll\");",
        "",
        "var win = new Window();",
        "win.setInnerSize(1920, 1080);",
        "win.caption = \"logo_test Wasmtime state trace\";",
        "",
        ...
        "",
        "function recordLogo(storage, chara, motion, frameCount)",
        "{",
        "    rm.load(storage);",
    ...
    out = work_dir / "motion_playback_state_trace.xp3"
    cmd = [str(xp3pack), "-o", str(out), "--map", f"startup.tjs={startup}"]
    for spec_id in mpb.SEGMENT_ORDER:
        spec = specs_by_id.get(spec_id)
        if spec is None:
            continue
        mtn_path = repo_root / str(spec["mtn_path"])
        if not mtn_path.exists():
            raise FileNotFoundError(f"motion fixture missing: {mtn_path}")
        cmd.append(f"{Path(str(spec['mtn_path'])).name}={mtn_path}")
...
def main(argv: list[str]) -> int:
    ...
    try:
        ...
        if framebuffer_dir is not None:
            ...
            temp_capture = tempfile.TemporaryDirectory(
                prefix="krkr2-motion-wasmtime-state-xp3-")
            trace_startup_xp3 = _build_state_trace_xp3(
