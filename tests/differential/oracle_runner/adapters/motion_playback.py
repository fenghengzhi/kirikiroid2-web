...
def _build_framebuffer_startup_tjs(
    ...
    lines = [
        ...
        f"var CAPTURE_START_FRAME = {capture_window.start};",
        f"var CAPTURE_END_EXCLUSIVE = {capture_window.end};",
        "var globalFrameId = 0;",
        ...
        "}",
        "",
        "function shouldCaptureFrame(frameId)",
        "{",
        "    return frameId >= CAPTURE_START_FRAME &&",
        "        frameId < CAPTURE_END_EXCLUSIVE;",
        "}",
        "",
        "function recordLogo(caseId, storage, chara, motion, frameCount)",
        "{",
        "    if (captureComplete || globalFrameId >= CAPTURE_END_EXCLUSIVE) {",
        ...
        "        player.draw(base);",
        "        if (shouldCaptureFrame(globalFrameId)) {",
        "            base.saveLayerImage(",
        "                CAPTURE_ROOT + \"/\" + caseId + \"/frame_\" + pad4(i) + \".png\",",
        "                \"png\");",
        "        }",
        "        globalFrameId++;",
        "        if (globalFrameId >= CAPTURE_END_EXCLUSIVE) {",
        "            captureComplete = true;",
        "            break;",
        "        }",
        "    }",
    ...
    for spec_id in SEGMENT_ORDER:
        ...
        if spec_id == "m2logo":
            lines.append(
                "if (!captureComplete) "
                "base.fillRect(0, 0, 1920, 1080, 0xFFFFFFFF);")
        storage = Path(str(spec["mtn_path"])).name
        lines.append(
            "if (!captureComplete) recordLogo("
...
def _build_framebuffer_capture_xp3(
    ...
    startup.write_text(
        _build_framebuffer_startup_tjs(
            specs_by_id, remote_capture_root, capture_window),
        encoding="utf-8",
    )
...
def _build_render_stage_startup_tjs(
    ...
    lines = [
        ...
        "        return;",
        "    }",
        "    var caseStartFrameId = globalFrameId;",
        "    var caseEndFrameId = caseStartFrameId + frameCount;",
        "    var captureCase = caseStartFrameId < CAPTURE_END_EXCLUSIVE &&",
        "        caseEndFrameId > CAPTURE_START_FRAME;",
        "    rm.load(storage);",
        "    var player = new Motion.Player(rm);",
        "    player.x = 960;",
        "    player.y = 540;",
        ...
        "    player.motion = motion;",
        "    player.play(motion, Motion.PlayFlagForce);",
        "    if (captureCase) savePhase(caseId, \"initial\", 0);",
        "    for (var i = 0; i < frameCount; i++) {",
        "        player.progress(FIXED_DELTA_MS);",
        "        player.draw(base);",
        "        if (shouldCaptureFrame(globalFrameId)) {",
...
def _build_render_stage_capture_xp3(
    ...
    startup = work_dir / "startup.tjs"
    startup.write_text(
        _build_render_stage_startup_tjs(
            specs_by_id, remote_capture_root, capture_window),
        encoding="utf-8",
    )
...
def _prepare_framebuffer_capture(
    ...
    local_xp3 = _build_framebuffer_capture_xp3(
        specs_by_id, remote_capture_root, work_dir, capture_window)
