...
| L4 | 0x6BF93C child ctor | PlayerUpdateParticles.cpp:446 | P2 | CORRECTED by fresh cross-check: binary 0x6BF950 writes `child+0=parent+0` (shared root/render owner), `child+8=parent` (immediate parent); `Player_ctor@0x6CED70` initializes root `+0=this`, and render/camera dereference +0 before draw-affine reads. The old “parent vtable copy / constructor-equivalent” interpretation was wrong; local must set both non-owning links after ctor and before CreateAdaptor. |
