...
| L7 | 0x6BE104 emote guard | PlayerUpdateChildMotion.cpp:14 | P2 | binary guards whole loop on `!*((_BYTE*)result+1092)` (player+1092 = isEmoteMode-ish). local `if(_isEmoteMode) return`. Confirm +1092 maps to _isEmoteMode (cluster E layout claims +1092). GOOD pending field map. |
