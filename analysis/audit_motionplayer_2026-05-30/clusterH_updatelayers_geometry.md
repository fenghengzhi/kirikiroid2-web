...
| H-11 | Player_calcBounds @0x6C3D04 | PlayerRenderItems.cpp:93-98 | P0 | gate mask: binary `v31 = preview(+1092)?5193:5185` (0x1449/0x1441) AND `node+200`(renderTreeFlag200). Local uses `0x1449/0x1441` — values match. TJS `completionType` is the independent +1144 int and does not select this mask. BUT binary ALSO skips when slot-done `node+536*slot+344` set (0x6C4028 outer `if(!...)`) and has the type4/type3 recursive paths BEFORE the mask path. Local mask path OK; missing slot-done gate + recursion ordering. |
