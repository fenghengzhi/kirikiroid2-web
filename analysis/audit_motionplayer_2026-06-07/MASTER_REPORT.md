...
- **J1/J7 普通主链 → ⚠ 2026-07-23 大部闭合但不外推 100%**：0x6C4E28 leaf/group pre-walk、SLA leaf owner、完整 float clip/Real size、Loop B child paintBox + 双 tuple、Variant tag、refresh gate，以及 0x6C7440 的 Layer-class receiver/target objthis、item-scope descriptor/color/source/accessor、RM.bufLayer、ancestor mask、异常 unwind 均已恢复。普通路径不再使用 `_renderLayerStates`；该字段只留于 HEADLESS accurate-SLA 诊断 J8。仍开放：`0x6C5264..0x6C532C` acquire caller payload 专用值类型、J8 accurate-SLA 持久树、0x6C7440 wrapper/executor 源码函数拆分。现有 logo 仍只可作 direct 非回归，不能冒充 leaf/parent-buffered Android oracle。
...
- **2026-06-07 同轮已落地**：J4（删 0x6C7440 中无二进制依据的 `renderLayer->Update(false)`）+ J9（priorDraw 下 signed `/2`），同样 build 绿、差分 0-mismatch、PNG 一致。
