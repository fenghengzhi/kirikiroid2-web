...
| `+304` `tTJSVariant` | Write: `sub_A0FB64(item+304, layerVariant)` after `sub_6C6B48` at `0x6C533C`; its `setSize/fillRect` and affine/Bezier/mesh copy belong to `0x6C4E28` Loop A. Read: `0x6C7440` consumes the already-materialized leaf output, including choosing `+304` in child alpha-mask traversal when `(item+244 & 4) == 0`. | First per-item layer-object variant (leaf output), **not** the final executor's work buffer. The buffered `0x6C7440` path gets its persistent work layer from Player+656 ResourceManager `PropGet(L"bufLayer")`. | `PreparedRenderItem::leafLayer`，由 build 原地写、execute 原地读；production build 的 target owner 在 SLA active/retired 双树中复用。 | `Captured` |
...
| `+324` `tTJSVariant` | `0x6C4E28` Loop B creates/fills this second layer object while composing group children. `0x6C7440` later chooses `child+324` instead of `child+304` when `(child+244 & 4) != 0` in the alpha-mask traversal around `0x6C8334`; it does not create the composed output there. | Second per-item layer-object variant (group/composed output), independent from both `+304` and RM `bufLayer`. | `PreparedRenderItem::composedLayer`，由 `composeGroupLayersLike_0x6C4E28` 写入，并与 `leafLayer` 保持独立 owner；execute 仍按 `(item+244 & 4)` 选择。 | `Captured` |
