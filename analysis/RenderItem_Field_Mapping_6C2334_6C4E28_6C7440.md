...
| `+21` `BYTE` | Write: set `1` when clip intersection succeeds at `0x6C4F88`; set `0` when it fails at `0x6C5E6C`. Read: second pass child traversal in `0x6C7440` requires `child+21` at `0x6C82F4`; parent union in `0x6C4E28` also reads it in the second half. | “This item has a valid clip rect / built local output for the current frame.” | Local runtime now preserves both cases in `RenderCommand::rawFlag21`: commands are instantiated before clip success is known, failed intersections stay in the local command list with `rawFlag21=0`, and execute/build paths gate on that bit instead of dropping the item during command creation. | `Captured` |
...
| `+232` `DWORD` | Read: `0x6C7440` top-level gate requires `item+232 != 0` at `0x6C75C8`. Write in `0x6C2334` from node-side data at `0x6C3610`. | Validity gate for a source-backed drawable path. | No explicit documented local one-to-one field; current port uses source/bitmap resolution plus `hasOwnSource`. | `Folded` |
...
1. Add explicit local storage for `node+200` and `node+201`.
2. Populate `PreparedRenderItem.skipFlag0/skipFlag1` from the exact `0x6C2334` formulas (`+17/+18`).
3. Continue tightening the remaining execute-path gates so `+17/+18/+20/+21` affect behavior exactly where the native path reads them, not just where the Web port happens to log them.
4. Trace and name the `+320` flag before changing any remaining alpha-mask/group behavior.
